    ...
    *,
    ...
    if node_positions is None:  # assign random starting positions to all nodes
        node_positions_as_array = (
            np.random.rand(total_nodes, dimensionality) * scale + origin
        )
    else:
        ...
        is_valid = _is_within_bbox(
            list(node_positions.values()), origin=origin, scale=scale
        )
...
def _fruchterman_reingold(
    adjacency, node_positions, origin, scale, temperature, k, node_radii
):
    """
    ...
    if (
        np.sum(distance == 0) - np.trace(distance == 0) > 0
    ):  # i.e. if off-diagonal entries in distance are zero
        warnings.warn(
            "Some nodes have the same position; repulsion between the nodes is undefined."
        )
    ...
    distance[
        distance <= 1e-6
    ] = 1e-6  # 1e-13 is numerical accuracy, and we will be taking the square shortly
    ...
    displacement = (
        displacement
        / displacement_length[:, None]
        * np.clip(displacement_length, None, temperature)[:, None]
    )
...
def _rescale_to_frame(node_positions, origin, scale):
    node_positions = (
        node_positions.copy()
    )  # force copy, as otherwise the `fixed_nodes` argument is effectively ignored
