...
def mutational_load(
  ...
  #              
                if frequency:
                    ...
                    rel_mutation = simple_hamming_distance(airr_df.iloc[row].loc[f"{chain}_{sequence_alignment}"],
                                                    airr_df.iloc[row].loc[f"{chain}_{germline_alignment}"],frequency=True)
                    mutations.append(rel_mutation)
                    
                else:
                    count_mutation = simple_hamming_distance(airr_df.iloc[row].loc[f"{chain}_{sequence_alignment}"],
    ...
    if region == "subregion":
        ...
        for chain in chains:
            ...
            for row in range(len(airr_df)):
                ...
                cdr3_germline = airr_df.iloc[row].loc[f"{chain}_{germline_alignment}"][312:(312+airr_df.iloc[row].loc[f"{chain}_junction_len"] - 6)]
                fwr4_germline = airr_df.iloc[row].loc[f"{chain}_{germline_alignment}"][(312+airr_df.iloc[row].loc[f"{chain}_junction_len"] - 6):]

                if frequency:
                    fwr1_mu_rel = simple_hamming_distance(subregion_df.iloc[row].loc[f"{chain}_fwr1"],fwr1_germline,frequency=True)
                    cdr1_mu_rel = simple_hamming_distance(subregion_df.iloc[row].loc[f"{chain}_cdr1"],cdr1_germline,frequency=True)
                    fwr2_mu_rel = simple_hamming_distance(subregion_df.iloc[row].loc[f"{chain}_fwr2"],fwr2_germline,frequency=True)
                    cdr2_mu_rel = simple_hamming_distance(subregion_df.iloc[row].loc[f"{chain}_cdr2"],cdr2_germline,frequency=True)
                    fwr3_mu_rel = simple_hamming_distance(subregion_df.iloc[row].loc[f"{chain}_fwr3"],fwr3_germline,frequency=True)
                    cdr3_mu_rel = simple_hamming_distance(subregion_df.iloc[row].loc[f"{chain}_cdr3"],cdr3_germline,frequency=True)
                    fwr4_mu_rel = simple_hamming_distance(subregion_df.iloc[row].loc[f"{chain}_fwr4"],fwr4_germline,frequency=True)
