...
def hill_diversity_profile(
    ...
    df = pd.DataFrame.from_dict(diversity, orient="index", columns=[a for a in np.arange(q_min, q_max+q_step, q_step)])
    df.index.name = groupby
    return df.T
     
def convert_hill_table(diversity_profile: pd.DataFrame,
                       convert_to: Literal["diversity", "evenness_factor",
                                           "relative_evenness"] = "diversity",
                       ) ->pd.DataFrame:
     """
    ...
    ----------
     ...
     if convert_to == "diversity":
        df = pd.DataFrame(columns= diversity_profile.columns, index= ["Observed richness", "Shannon entropy", "Inverse Simpson", "Gini-Simpson"])
