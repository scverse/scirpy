...
def processResultsOf10xRepSeq(prefix, contigF, consenseF, cellF, chainF, cdrF):
    ...
    contigHeader, contigTable, sampleCells = readContigTable(prefix, contigF, contigTable, sampleCells)
    ...
    consensusTable, sampleChains = readConsensusTable(prefix, consenseF, consensusTable, sampleChains)
    ...
    chainHeader, chainTable, consensusTable = harmonizeContgisAndConsensus(prefix, sampleCells, sampleChains, contigHeader, contigTable, consensusTable)
    ...
    cellHeader = ['barcode', 'sample', 'clonotype', 'chainPairing', 'fragmentChains',
        'dominantAreads', 'dominantAcdrlen', 'dominantAaddnuc', 'dominantAcontig', 'dominantAfragment', 'dominantAgeneV', 'dominantAgeneD', 'dominantAgeneJ',
        'dominantBreads', 'dominantBcdrlen', 'dominantBaddnuc', 'dominantBcontig', 'dominantBfragment', 'dominantBgeneV', 'dominantBgeneD', 'dominantBgeneJ',
        'secondaryAreads', 'secondaryAcdrlen', 'secondaryAaddnuc', 'secondaryAcontig', 'secondaryAfragment', 'secondaryAgeneV', 'secondaryAgeneD', 'secondaryAgeneJ',
        'secondaryBreads', 'secondaryBcdrlen', 'secondaryBaddnuc', 'secondaryBcontig', 'secondaryBfragment', 'secondaryBgeneV', 'secondaryBgeneD', 'secondaryBgeneJ'
        ]
    for cell in sampleCells.keys():
        ...
        cellchains = summarizeChainInfoForCell(cellContigs, consensusTable, contigTable, contigHeader, sampleChains, cellchains)
...
def readConsensusTable(prefix, consenseF, consensusTable, sampleChains):
    with open(consenseF) as f:
        ...
        for clone in data:
            ...
            inserted, segments, segline, chain = collectSegments(contid, clone['annotations'], cdrStart, cdrStop)
...
def harmonizeContgisAndConsensus(prefix, sampleCells, sampleChains, contigHeader, contigTable, consensusTable):
    chainTable = []
    fragment_n = 1000
    chainHeader = list(contigHeader.items())
    chainHeader.sort(key=lambda x: x[1])
    chainHeader = [x[0] for x in chainHeader] + ['nseq', 'aseq', 'ncdr', 'acdr', 'cdrlen', 'addednuc', 'sample', 'segments']
    chainHeader = chainHeader[:3] + chainHeader[4:13] + chainHeader[15:17] + chainHeader[19:]
    for key, ctg in contigTable.items():
        ctg = [key] + ctg
        seginfo = []
        if key in sampleChains:
            supplements = consensusTable[sampleChains[key]]
            spp = [supplements['nseq'], supplements['aseq'], supplements['ncdr'], supplements['acdr'], supplements['cdrlen'], supplements['addednuc']]
            ...
            consensusTable[fkey] = {'nonproductive': True, 'ncdr': '', 'acdr': '', 'chain': ctg[6], 'segline': [ctg[7].upper(), ctg[8].upper(), ctg[9].upper()], 'cdrlen': 0, 'addednuc': 0}
...
def summarizeChainInfoForCell(cellContigs, consensusTable, contigTable, contigHeader, sampleChains, cellchains):
    for ctg in cellContigs:
        ...
        cellinf = [cdr3, cdr3_nt, reads, cdrlen, addednuc, ctg, frg] + arowdata['segline']
        if chain in ['TRA', 'TRB']:
            if cellchains[chain][0] == '':
                ...
                if reads > cellchains[chain][2] :
                    if cdr3 == '':
                        if cellchains['_'+chain] == '' and reads > cellchains['_'+chain][2]:
...
def calculateConvergenceStats(convergeTable, prefix):
    ...
    for aa, nt in convergeTable.items():
        if aa != '':
            ...
            convergeStat.append([aa, prefix, ntdVars, sumOccurrence, positionFidelity, dominanceFidelity, alphaConv, betaConv, domConv, secConv, alphaOccurrence, betaOccurrence, dominantOccurrence, secondaryOccurrence, domApOccurrence, secApOccurrence, domBpOccurrence, secBpOccurrence, alphaRatio, betaRatio, domRatio, secRatio])
    return convergeStat
