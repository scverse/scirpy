...
def __main__():
    ...
    if main_semaphore == "celldist":
        ...
        if len(chains) != disTool.num_obs_y(condensedDistances):
            print(
                "Matrix size incompatible with number of observations! Need a condensed distance matrix, not a squareform!"
            )
            return
        chainsOnCells = readMap(
            cellChains, bottlneck=chains, disambiguation=disambiguation
        )
        cellDistances, cells = calculateCellDistanceFromChains(
            condensedDistances,
            chains,
            chainsOnCells,
        ...
        saveDistMatToHf5(
            cellDistances, cells, distanceOut, disType="condensedCelldist_"
        )
        readDistances(distanceOut)
...
def squareFromCondensed(distances, numCores=2):
    ...
    M = p.map(
        fillSquareDist,
        itertools.zip_longest(range(0, d), [], fillvalue=(distances, d)),
        chunksize=chunkSize,
    )
    p.close()
    p.join()
...
def calculateCellDistanceFromChains(
    condensedDistances, chains, chainsOnCells, disambiguation="minimum", numCores=2
):
    ...
    M = p.map(
        ...
        itertools.zip_longest(
            itertools.combinations(cellnames, 2),
            [],
            fillvalue=(condensedDistances, chainsOnCells, posDict, disambiguation, L),
        ),
        chunksize=chunkSize,
    )
    p.close()
...
def cdrSeqBasedDistanceCalculation(chainSeqs, penalties=[-10, -2], numCores=2):
    ...
    p.map(
        managedSelfSimFill,
        itertools.zip_longest(chainSeqs, [], fillvalue=(matrix, selfSimilarities)),
        chunksize=chunkSize,
    )
    p.close()
    p.join()
    numCores, chunkSize = setChunkSize(numCores, N * (N - 1))
    p = manager.Pool(numCores)
    distanceMatrix = p.map(
        managedDistCalc,
        itertools.zip_longest(
            itertools.combinations(chainSeqs, 2),
            [],
            fillvalue=(matrix, gap_open, gap_extend, selfSimilarities),
        ),
        chunksize=chunkSize,
    )
    p.close()
...
def managedSelfSimFill(arglist):
    ...
    selfSimilarities[seq[0]] = sum(
        [matrix[(c, c)] for c in seq[1] + seq[2] + seq[1] + seq[2]]
    )
    return
...
def managedDistCalc(arglist):
    ...
    similarity = distanceOfCDR3Seqs(
        a1, a2, b1, b2, matrix, gap_open, gap_extend, normFactor=max(s1, s2)
    )
    return similarity


def distanceOfCDR3Seqs(a1, a2, b1, b2, matrix, gap_open, gap_extend, normFactor=1):
    similarity_score = pairwise2.align.globalds(
        a1, a2, matrix, gap_open, gap_extend, one_alignment_only=True, score_only=True
    ) + pairwise2.align.globalds(
        b1, b2, matrix, gap_open, gap_extend, one_alignment_only=True, score_only=True
    )
