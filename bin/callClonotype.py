...
def __main__():
    none, cdrF, clonInfo, cellInfo, convergenceTab, chainMap, chainPairs, chainNet, seqsToDiv, seqsToDis = sys.argv
...
def callClonotypesAndChainPairs(cdrF, clonInfo, cellInfo, convergenceTab, chainMap, chainPairs, chainNet, seqsToDiv, seqsToDis, distance_method='minimum'):
    ...
    chainMapping, cdrMapping, convergenceTable, clonoFreq, cellChainTable, cellInfoTable, chainsForDiversity, chainsForDist = renameChain(cdrF, chainMapping, cdrMapping, convergenceTable, clonoFreq, cellChainTable, cellInfoTable, chainsForDiversity, chainsForDist, distance_method)
    ...
    sharedChains, clonotypePerSample, samples, cellTable = countClonotypesInSamples(cellInfoTable, clonotypeMapping, sharedChains)
    ...
    convergenceStats = detailedConvergenceStatistics(convergenceTable, cdrMapping, convergenceStats)
    f = open(convergenceTab, 'w')
    header = '\t'.join(convergenceStats.pop('header', ['NA']))
    f.write('\n'.join([header] + ['\t'.join([k]+[str(y) for y in x]) for k, x in convergenceStats.items()])+'\n')
    f.close()

    #Add some convergence statistics to clonotypes
    clonotypeConvergence = clonotypeConvergenceStats(clonotypeMapping, convergenceStats, set(cdrMapping['XXXXXXXXXXXX'][1]))

    #Assign chain convergence information and sample-wise abundance to clonotypes
    clonotypeInfoTable = reformatClonotypeAbundance(clonotypeConvergence, clonotypePerSample, samples)
    f = open(clonInfo, 'w')
...
def renameChain(cdrF, chainMapping, cdrMapping, convergenceTable, clonoFreq, cellChainTable, cellInfoTable, chainsForDiversity, chainsForDist, distance_method):
    ...
    with open(cdrF) as f:
        for line in f:
            ...
            clonoSign, a1aa, b1aa, a2aa, b2aa, a1nt, b1nt, a2nt, b2nt, a1ctg, b1ctg, a2ctg, b2ctg = line
            ...
            if len(cellNames) == 1:
                ...
                cellChains.append([cellNames, 'primary', chainLinks[0]+':'+chainLinks[1]])
                cellChains.append([cellNames, 'secondary', chainLinks[2]+':'+chainLinks[3]])
                if chainLinks[2]+':'+chainLinks[3] != '10000.1.A.2:10000.1.B.2':
                    cellChains.append([cellNames, 'swap', chainLinks[0]+':'+chainLinks[3]])
                    cellChains.append([cellNames, 'swap', chainLinks[2]+':'+chainLinks[1]])
    ...
    return chainMapping, cdrMapping, convergenceTable, clonoFreq, cellChainTable, cellInfoTable, chainsForDiversity, chainsForDist
...
def countClonotypesInSamples(cellInfoTable, clonotypeMapping, sharedChains):
    ...
    for cell, cellinfo in cellInfoTable.items():
        ...
        cellTable.append([cell, signiture, ':'.join(chains), ':'.join(chainmains)] + chains + chainmains + [chains[0]+':'+chains[1], chains[2]+':'+chains[3], chains[0]+':'+chains[2], chains[1]+':'+chains[3]] + [chainmains[0]+':'+chainmains[1], chainmains[2]+':'+chainmains[3], chainmains[0]+':'+chainmains[3], chainmains[2]+':'+chainmains[1], chainmains[0]+':'+chainmains[2], chainmains[1]+':'+chainmains[3]])
    return sharedChains, clonotypePerSample, samples, cellTable

def detailedConvergenceStatistics(convergenceTable, cdrMapping, convergenceStats=[], chainNames=['A.1', 'B.1', 'A.2', 'B.2'], lineprefix='', includeHeaders=True):
    headrow = [
        'aaSeq',
        'cells',
        'chainID',
    ...
    for aa, nt in convergenceTable.items():
        ...
        if listForm:
            convergenceStats.append([lineprefix, aa, cellist, chaincode, ntdVars, sumOccurrence, positionFidelity, dominanceFidelity, alphaConv, betaConv, domConv, secConv, alphaOccurrence, betaOccurrence, dominantOccurrence, secondaryOccurrence, domApOccurrence, secApOccurrence, domBpOccurrence, secBpOccurrence, alphaRatio, betaRatio, domRatio, secRatio])
        else:
            convergenceStats[aa] = [cellist, chaincode, ntdVars, sumOccurrence, positionFidelity, dominanceFidelity, alphaConv, betaConv, domConv, secConv, alphaOccurrence, betaOccurrence, dominantOccurrence, secondaryOccurrence, domApOccurrence, secApOccurrence, domBpOccurrence, secBpOccurrence, alphaRatio, betaRatio, domRatio, secRatio]
    if includeHeaders:
        if listForm:
...
def reformatClonotypeAbundance(clonotypeConvergence, clonotypePerSample, samples):
    ...
    M = [['Clonotype', 'Signiture', 'Cells', 'cdrIdA1', 'nucvarA1', 'numchainA1', 'positionA1', 'dominanceA1', 'cdrIdB1', 'nucvarB1', 'numchainB1', 'positionB1', 'dominanceB1', 'cdrIdA2', 'nucvarA2', 'numchainA2', 'positionA2', 'dominanceA2', 'cdrIdB2', 'nucvarB2', 'numchainB2', 'positionB2', 'dominanceB2']+samples]+M
    return M
